...
int main(int argc, char *argv[]) {
  ...
  *load_config.MutableExtension(asylo::sgx_load_config) = sgx_config;
  ...
  for (const auto &name : names) {
	...
	unsigned char *pStr = (unsigned char *)name.c_str();
	unsigned long length = name.length();

//	compress & set input
	unsigned long nCompressedSize = length;
	unsigned char *pCompressedData = new unsigned char [nCompressedSize];

	int nResult = compress(pCompressedData, &nCompressedSize, pStr, length);
	if (nResult != Z_OK) {
		LOG(INFO) << "Compression failed " << nResult << "\n" ;
		LOG(INFO) << "Compression orig " << length << " bytes --> " << nCompressedSize << " bytes\n" ;
	} else  {
		LOG(INFO) << "Compression succeed " << nResult << "\n" ;
		LOG(INFO) << "Compression orig " << length << " bytes --> " << nCompressedSize << " bytes\n" ;
		LOG(INFO) << "orig " << pStr << " --> " << pCompressedData << "\n" ;
	}

/*
    input.MutableExtension(compress_world::enclave_input_hello)
        ->set_to_greet(pStr, length);
*/
    input.MutableExtension(compress_world::enclave_input_hello)
        ->set_to_greet(pCompressedData, nCompressedSize);
    ...
    if (!output.HasExtension(compress_world::enclave_output_hello)) {
      LOG(QFATAL) << "Enclave did not assign an ID for " << name;
    }

    std::cout << "Message from enclave: "
              << output.GetExtension(compress_world::enclave_output_hello)
                     .greeting_message()
              << std::endl;
