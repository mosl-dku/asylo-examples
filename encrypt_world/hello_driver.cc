...
int main(int argc, char *argv[]) {
  ...
  *load_config.MutableExtension(asylo::sgx_load_config) = sgx_config;
  ...
  for (const auto &name : names) {
    ...
    input.MutableExtension(encrypt_world::enclave_input_hello)
        ->set_to_greet(name);
    ...
    if (!output.HasExtension(encrypt_world::enclave_output_hello)) {
      LOG(QFATAL) << "Enclave did not assign an ID for " << name;
    }

    std::cout << "Message from enclave: "
              << output.GetExtension(encrypt_world::enclave_output_hello)
                     .greeting_message()
              << std::endl;
  }
